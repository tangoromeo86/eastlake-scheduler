...
// A team's own start date — everything before it is unconditionally
// unavailable (lib/scheduler.js's resolveTeamAvailability). Format-only check,
// same as season start / blackout dates elsewhere — no season-range
// cross-check, so this doesn't need season data threaded through it.
function validateEarliestDate(raw) {
  if (raw === undefined || raw === null || raw === '') return { ok: true, value: undefined };
...
// Jersey color swatch — a plain #rrggbb hex, always optional (no default
// means "no color set yet," not an error).
function validateJerseyColor(raw) {
  if (raw === undefined || raw === null || raw === '') return { ok: true, value: '' };
  const value = String(raw).trim();
  if (!/^#[0-9a-fA-F]{6}$/.test(value)) {
    return { ok: false, error: 'Jersey color must be a hex value like #1a3c8f' };
  }
  return { ok: true, value: value.toLowerCase() };
}

// The short text that goes alongside the swatch ("Navy", "Away White") —
// real jerseys are rarely one clean named color, so this is free text
// rather than a fixed palette, same reasoning as validateName but shorter
// since it's meant to read inline on a schedule row.
function validateJerseyLabel(raw) {
  return validateName(raw, { label: 'Jersey label', max: 30, required: false });
}
